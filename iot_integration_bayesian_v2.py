...
========================================================================
Extends ADACON [Bhore et al., 2026] to heterogeneous IoT environments.
...
# ADACON hysteresis-corrected switching baseline (Table 3, Bhore et al. 2026).
# The pre-hysteresis figure (699) must NOT be used for reduction claims.
...
class BayesianThreatDetector:
    """
    Extends ADACON's Bayesian threat detection [Bhore et al., 2026] to
...
class DomainAwareConsensusEngine:
    ...
    def _get_weight_profile(self, state: NetworkState,
        ...
        elif max_threat > 0.35:   # was 0.60 — too high for IoT posteriors
...
class StabilityAwareConsensusLayer:
    ...
    Traffic:     min 20 steps dwell; 6% improvement threshold
    Environment: min 35 steps dwell; 6% improvement threshold
    ...
    # optimum (Table 4, Bhore et al. 2026) while preserving IoT-domain stability.
    # Under active high-confidence threat, dwell is further halved (see update()).
    MIN_DWELL = {
        IoTDomain.TRAFFIC:     10,   # was 20 — caused <5 switches per 1,000 steps
        IoTDomain.ENVIRONMENT: 15,   # was 35 — excessive for LoRaWAN attack cadence
    }
    ...
    IMPROVEMENT_THRESHOLD = 0.08   # was 0.06; tighter to avoid trivial flipping
...
class TrafficNetworkSimulator:
    ...
    def generate_network_state(self) -> NetworkState:
        ...
        if self._attack == 'dos':
            ...
            error_rate  += random.gauss(0.35, 0.05)   # was 0.20
            packet_loss += random.gauss(0.08, 0.015)   # was 0.05
            latency     += random.gauss(0.10, 0.02)    # extra re-broadcast delay
            cpu_usage    = min(0.95, cpu_usage + 0.25) # was 0.20
        elif self._attack == 'eclipse':
            # Strengthened: isolated nodes see stale chain, high latency & errors
            latency     += random.gauss(0.35, 0.06)   # was 0.20
            packet_loss += random.gauss(0.10, 0.02)   # was 0.06
            ...
            error_rate  += random.gauss(0.28, 0.05)   # was 0.15
            ...
            latency     += random.gauss(0.40, 0.07)   # was 0.30
            packet_loss += random.gauss(0.08, 0.015)  # was 0.04
...
class EnvironmentalNetworkSimulator:
    ...
    def generate_network_state(self) -> NetworkState:
        ...
        if self._attack == 'dos':
            ...
            error_rate  += random.gauss(0.35, 0.06)      # was 0.20
            packet_loss += random.gauss(0.08, 0.015)
            ...
            cpu_usage    = min(0.85, cpu_usage + 0.20)   # was 0.30 cpu only
        elif self._attack == 'routing':
    ...
    def generate_domain_comparison_table(self, t_res: Dict, e_res: Dict) -> pd.DataFrame:
        ...
        for label, res in [('Traffic Management', t_res),
            ...
            rows.append({
                ...
                'Threat Prevention': f"{round(np.mean(res['threat_prevention'])*100, 1)}%",
...
def run_static_baseline(domain: IoTDomain, mechanism: ConsensusType,
    ...
    latencies, throughputs, energies, preventions = [], [], [], []
    active_attack = None
    ...
    for i in range(time_steps):
        ...
        # random.uniform draws so both sides of every comparison are
        # measured identically (fair-comparison fix, Reviewer 2, R2).
        if active_attack:
            appropriate = {
            ...
            preventions.append(float(np.clip(
    ...
    return {
        ...
        'threat_prevention': np.mean(preventions),
    }
...
def run_simulation(domain: IoTDomain, time_steps: int = 1000,
    ...
    history = {
        ...
        'threat_probs': [], 'mechanism': [], 'score': [], 'threat_prevention': [],
        # Reviewer fix 3 — concrete attack outcome metrics (independent observables)
    ...
    for i in range(time_steps):
        ...
        # ── Reviewer fix 3: physics-based attack outcome metrics ─────────────
        # Replaces random.uniform draws with deterministic formulae grounded in
        # the network state, so prevention is an independent observable rather
        ...
        #
        # These are combined into a single prevention rate in [0,1].
        if active_attack:
            ...
            prevention = float(np.clip(raw_prevention, 0.05, 0.99))
        ...
        history['threat_prevention'].append(prevention)
    ...
    return {
        ...
        'threat_prevention':  history['threat_prevention'],
...
def plot_results(traffic_results: Dict, env_results: Dict,
    ...
    _fig6_threat_prevention_bar(traffic_results, env_results, traffic_static, env_static)
...
# ── Figure 6: Threat prevention bar — both domains ───────────────────────────
def _fig6_threat_prevention_bar(traffic_results, env_results, traffic_static, env_static):
    labels = MECH_ORDER
    t_adapt = np.mean(traffic_results['threat_prevention'])
    e_adapt = np.mean(env_results['threat_prevention'])
    t_vals  = [traffic_static[m]['threat_prevention'] if m in traffic_static else t_adapt for m in labels]
    e_vals  = [env_static[m]['threat_prevention']     if m in env_static     else e_adapt for m in labels]
...
def _fig7_static_comparison_full(traffic_results, env_results, traffic_static, env_static):
    ...
    metrics = {
        ...
        'Mitigation Response — Traffic\n(higher = better)': {
            m: traffic_static[m]['threat_prevention'] if m in traffic_static
               else np.mean(traffic_results['threat_prevention']) for m in labels},
    }
...
# ── Figure 11: REMOVED ────────────────────────────────────────────────────────
# The former threat-prevention heatmap plotted a hard-coded matrix described as
...
# Matches ADACON's validation protocol (seeds 5-9, Table 11, Bhore et al. 2026)
# =============================================================================
...
def run_multi_seed_validation(domain: IoTDomain, time_steps: int = 1000) -> Dict:
    ...
    for seed in VALIDATION_SEEDS:
        ...
        records.append({
            ...
            'threat_prev_mean':  float(np.mean(r['threat_prevention'])),
        ...
        print(f"    seed={seed}: lat={records[-1]['latency_mean']:.4f}  "
              ...
              f"prev={records[-1]['threat_prev_mean']*100:.1f}%  "
              f"switches={records[-1]['n_switches']}")

    df = pd.DataFrame(records)
    agg = {}
    for col in ['latency_mean','throughput_mean','threat_prev_mean',
...
def _print_validation_table(agg: Dict, domain_label: str):
    ...
    metric_labels = {
        ...
        'threat_prev_mean':  'Threat Prevention Rate',
    ...
    for key, label in metric_labels.items():
        a = agg[key]
        is_pct = key in ('threat_prev_mean',)
...
def run_concurrent_attack_scenario(domain: IoTDomain,
    ...
    print(f"    Avg prevention (concurrent attack): {avg_prev_attack*100:.1f}%")
    print(f"    Avg prevention (baseline periods) : {avg_prev_baseline*100:.1f}%")
...
def main():
    ...
    print("  Extends ADACON [Bhore et al., 2026] to Heterogeneous IoT")
    ...
    print("  Reviewer fixes applied: [1] score∈[0,1]  [2] Env switching  [3] outcome")
    ...
    # ── Reviewer fix 6: PBFT scalability note ────────────────────────────────
    ...
    for domain_label, static_res, adapt_res in [
        ...
        for name, r in static_res.items():
            rows.append({
                ...
                'Threat Prevention': f"{r['threat_prevention']*100:.1f}%",
                'Switches':          0,
            })
        rows.append({
            ...
            'Threat Prevention': f"{np.mean(adapt_res['threat_prevention'])*100:.1f}%",
    ...
    print("  Independent observables replacing random.uniform prevention scores")
    print("="*65)
    for label, res in [('Traffic Management', traffic_results),
        ...
        tp  = np.mean(res['threat_prevention'])
        print(f"\n  {label}:")
        outcome_rows = [
    ...
    # ── Reviewer fix 4: Multi-seed validation ────────────────────────────────
    print(f"\n{'='*65}")
    print("MULTI-SEED STATISTICAL VALIDATION  (5 independent seeds: 5-9)")
    print("  Matches ADACON validation protocol — Bhore et al. 2026, Table 11")
    print("="*65)
    ...
    # ── Reviewer fix 5: Concurrent multi-vector attack ───────────────────────
    print(f"\n{'='*65}")
    ...
    summary = [
        ...
        ['  Threat prevention',     f"{t_agg['threat_prev_mean']['mean']*100:.2f}% ± {t_agg['threat_prev_mean']['std']*100:.2f}%"],
        ...
        ['  Threat prevention',     f"{e_agg['threat_prev_mean']['mean']*100:.2f}% ± {e_agg['threat_prev_mean']['std']*100:.2f}%"],
...
def _fig_concurrent(t_conc: Dict, e_conc: Dict):
    """Figure 12 — Concurrent Sybil+DoS attack: mechanism timeline & prevention."""
...
def _fig_multi_seed(t_val: Dict, e_val: Dict):
    ...
    metrics = ['latency_mean', 'throughput_mean', 'threat_prev_mean',
...
def _fig_outcome_metrics(traffic_results: Dict, env_results: Dict):
    ...
    fig.suptitle("Figure 14 — Concrete Attack Outcome Metrics Over Time\n"
                 "(independent observables replacing random.uniform prevention scores)",
                 fontsize=12, fontweight='bold')
