...
Usage:   python run_sensitivity.py   (needs iot_integration_bayesian_v4.py alongside)
"""
...
def run_config(domain: IoTDomain) -> dict:
    ...
    for seed in VALIDATION_SEEDS:
        ...
        rows.append({
            ...
            'mitigation': float(np.mean(r['posture_index'])),
...
def main():
    ...
    sens.to_csv(os.path.join(RESULTS, 'sensitivity_summary.csv'), index=False)
    print(pretty(sens))
    ...
    abl.to_csv(os.path.join(RESULTS, 'ablation_summary.csv'), index=False)
    print(pretty(abl))
